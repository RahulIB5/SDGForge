...
const handleSendMessage = useCallback(
  async (message: string) => {
    ...
    try {
      const res = await fetch('http://localhost:8000/legal/ask', {
        method: 'POST',
      ...
      const assistantText = data?.answer ?? "Sorry, I couldn't process that.";
      ...
      if (!isSpeaking) speak({ text: assistantText });
    } catch (error) {
      console.error('Voice assistant error:', error);
      toast({
        title: "Assistant Error",
  ...
  [speak, isSpeaking, toast]
);


  const handleVoiceInput = () => {
    if (listening) stopListening();
    else listen({ lang: currentLanguage.code });
  };
